      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {allProducts?.map((product) => (
          <div
            ...
            className="group cursor-pointer rounded-2xl overflow-hidden p-4 hover:shadow-lg transition"
          >
            {/* Image and Product Info - Clickable Link */}
            <Link href={`/product/${product.id}`} className="block">
              ...
              <div className="relative aspect-square overflow-hidden rounded-lg bg-gray-100 mb-4">
                ...
                {/* Wishlist - Fixed Event Handling */}
                <button
                  onClick={(e) => handleWishlistClick(e, product.id)}
                  className="absolute top-3 right-3 p-2 bg-white/90 rounded-full shadow hover:bg-white z-10"
            ...
            {/* Cart Button - Outside Link, Separate Click Area */}
            <div className="mt-3">
              {cart.some((c) => c.id === product.id) ? (
                <button
                  className="w-36 text-sm rounded-3xl bg-gradient-to-r from-[#F35C7A] to-[#a30b29] 
